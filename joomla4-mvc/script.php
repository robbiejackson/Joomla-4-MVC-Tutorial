...
class com_helloWorldInstallerScript
{
    ...
    function postflight($type, $parent) 
    {
        ...
        echo '<p>Checking if the config parameters have been set ...</p>';
        
        $query = $db->getQuery(true);
		$query->select('params');
		$query->from('#__extensions');
		$query->where('element = "com_helloworld"');
		$db->setQuery($query);
        ...
        if (count($params) < 6)
        {
            $defaults = array("show_category"=>"0", "user_to_email"=>"0", "save_history"=>"1", 
                            "history_limit_helloworld"=>"3", "history_limit_category"=>"2", "show_feed_link"=>"1");
